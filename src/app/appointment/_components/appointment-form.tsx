...
import { step1Schema, step2Schema, step3Schema, step4Schema } from '@/schemas/appointment'
...
export default function AppointmentForm () {
  ...
  const userObj: User = userData ?? { image: '', name: '', email: '', phone_number: '' }
        ...
        <CardContent>
                ...
                <Fragment key={step.id}>
                  <li className="flex items-center gap-4 flex-shrink-0">
                    <Button
                      ...
                      className={cn("flex size-10 items-center justify-center rounded-full hover:bg-violet-500 hover:text-white", index <= currentIndex ? 'bg-violet-500' : '')}
                      onClick={async () => {
                  ...
                  {index < array.length - 1 && (
                    <Separator className={`flex-1 ${index < currentIndex ? 'bg-violet-500' : 'bg-muted'}`} />
                  )}
                </Fragment>
              ))}
            </ol>
          ...
          <div className="space-y-8">
            {stepper.switch({
              ...
              summary: () => <AppointmentSummary goToStep={(stepIdx: number) => stepper.goTo(steps[stepIdx].id)} user={userObj} pets={petsList} />,
            })}
            <CardFooter>
              <div className="flex justify-end gap-4">
                <Button variant="secondary" onClick={stepper.prev} disabled={stepper.isFirst} type="button">
                  Anterior
                </Button>
                <Button type="submit">

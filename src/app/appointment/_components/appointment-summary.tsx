...
const doctors = [
  ...
  },
  {
    id: "dr. garcía",
    nombre: "Dr. García",
    foto: "https://avatar.iran.liara.run/public/boy?username=garcia",
  },
  {
    id: "dra. rodríguez",
    nombre: "Dra. Rodríguez",
    foto: "https://avatar.iran.liara.run/public/girl?username=rodriguez",
  },
  {
    id: "dr. lópez",
    nombre: "Dr. López",
    foto: "https://avatar.iran.liara.run/public/boy?username=lopez",
  },
  {
    id: "dra. martínez",
    nombre: "Dra. Martínez",
    foto: "https://avatar.iran.liara.run/public/girl?username=martinez",
  },
];
...
export default function AppointmentSummary({
  ...
  const selectedPet = pets.find(
    (pet) => pet.id.toString() === formData.selectedPetId
  );
    ...
    <div className="space-y-6">
          ...
          {formData.petSelection === "existing" && selectedPet ? (
            ...
            <>
              ...
              <div>
                ...
                <p>
                  <strong>Nombre:</strong> {formData.petName}
                </p>
                {formData.petType !== "OTHER" ? (
                  <p>
                    <strong>Raza:</strong> {formData.petBreed}
                  </p>
                ) : (
                  <p>
                    <strong>Descripción:</strong> {formData.petDescription}
                  </p>
                )}
                {formData.petAge && (
                  <p>
                    <strong>Edad:</strong> {formData.petAge}
                  </p>
                )}
                {formData.petWeight && (
                  <p>
                    <strong>Peso:</strong> {formData.petWeight}
                  </p>
                )}
              </div>
            </>
      ...
      >
        <div className="text-sm space-y-1 text-muted-foreground">
          <p>
            ...
            {formData.serviceType === "baño"
              ? "Baño"
              : formData.serviceType === "corteBaño"
          ...
          {formData.serviceType === "visitaMedica" && (
            <>
              <p>
                <strong>Razón:</strong> {formData.visitReason}
              </p>
              {formData.visitDetails && (
                <p>
                  <strong>Detalles:</strong> {formData.visitDetails}
                </p>
              )}
            </>
          )}
      ...
      >
        <div className="flex items-center gap-4 text-muted-foreground">
          ...
          <div>
            <p>
              <strong>Profesional:</strong> {formData.doctor}
            </p>
            <p>
              <strong>Fecha:</strong> {formData.appointmentDate}
            </p>
            <p>
              <strong>Hora:</strong> {formData.appointmentTime}
            </p>
          </div>
        </div>
      </ResumenSeccion>
